...
func (p *PayerObject) QGetPayer(db *sql.DB) error {
	return db.QueryRow(`SELECT * FROM payer WHERE id=$1 LEFT JOIN address ON address.payer=payer.id`,
		p.ID).Scan(&p.ID, &p.Address)
}
...
func (p *PayerObject) QGetPayers(db *sql.DB, start int, count int) ([]PayerObject, error) {
	rows, err := db.Query(
		...
		payer.user_reference FROM payer LEFT JOIN address ON address.payer=payer.id LIMIT $1 OFFSET $2`,
		count, start)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&payer.ID, &payer.Name, &payer.Email, &payer.BirthDate, &payer.Phone,
			&payer.Document, &payer.UserReference, &payer.CreatedAt, &p.Address.State,
			&p.Address.City, &p.Address.ZipCode, &p.Address.Street, &p.Address.Number); err != nil {
			return nil, err
		}
